...
let { ipcRenderer, webContents } = require("electron");
...
if (auth == undefined) {
  ...
  $(document).ready(function () {
    ...
    $.fn.editProduct = function (index) {
      ...
      $("#purchase_price").val(allProducts[index].purchase);
  ...
  $.get(api + query, function (transactions) {
    if (transactions.length > 0) {
      ...
      transactions.forEach((trans, index) => {
        ...
        let profitTag = `<h5>Profit</h5> <div id='counter'>${
          settings.symbol + parseFloat(totalProfit).toFixed(2)
        }</div>`;
        $("#total_products").html(profitTag);
        //    <td>${
        //                                   trans.paid == ""
        //                                     ? ""
